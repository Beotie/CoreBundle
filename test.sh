...
runner "/usr/bin/env php vendor/bin/phpcbf --standard=./csruleset.xml CoreBundle/"
echo "$TEST_RES" >> doc/phpcbf.txt

test "/usr/bin/env php vendor/bin/phpunit" PHPUnit 100
echo "$TEST_RES" > doc/phpunit.txt

test "/usr/bin/env php vendor/bin/phpcs --standard=./csruleset.xml CoreBundle/" PHPCS 100
echo "$TEST_RES" > doc/phpcs.txt

test "/composer/composer.phar validate" COMPOSER 100
echo "$TEST_RES" > doc/composer.txt

test "/usr/bin/env php vendor/bin/phpmd CoreBundle/ text ./phpmd.xml" PHPMD 100
echo "$TEST_RES" > doc/phpmd.txt

test "/usr/bin/env php vendor/bin/phpcpd CoreBundle/" PHPCPD 1
echo "$TEST_RES" > doc/phpcpd.txt

test "/usr/bin/env php vendor/bin/phpmetrics --report-html=doc/metrics CoreBundle/" PHPMETRICS 1
