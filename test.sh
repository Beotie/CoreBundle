...
test "$PHP_PATH vendor/bin/phpcs --standard=./csruleset.xml CoreBundle/" PHPCS 100
echo "$TEST_RES" > doc/phpcs.txt

test "$COMPOSER_PATH validate" COMPOSER 100
echo "$TEST_RES" > doc/composer.txt

test "$PHP_PATH vendor/bin/phpmd CoreBundle/ text ./phpmd.xml" PHPMD 100
echo "$TEST_RES" > doc/phpmd.txt

test "$PHP_PATH vendor/bin/phpcpd CoreBundle/" PHPCPD 1
echo "$TEST_RES" > doc/phpcpd.txt

test "$PHP_PATH vendor/bin/phpmetrics --report-html=doc/metrics CoreBundle/" PHPMETRICS 1
